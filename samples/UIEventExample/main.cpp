...
int main()
{
    ...
    renderingSystem->UIRenderEvent += [&](auto system, DefaultRenderingSystem::UIRenderEventArgs args)
    {
        ...
        {
            //this is the quickest way to get the constant buffer region that the shader expects since we don't rely on ECS transform data here.
            transformRegion = args.resourceManager.AllocateConstantBufferRegion(sizeof(NovelRT::Maths::GeoMatrix4x4F) * 1000);
        ...
        //nothing else will touch this so we can just access the concurrent pointer in this example without a lock. Usually you'd need a std::scoped_guard however.
        inputResourceRegions.emplace_back(redTextureFuture.GetValue().gpuTextureRegion);

        primitive = args.graphicsDevice->CreatePrimitive(pipeline->gpuPipeline.GetUnderlyingSharedPtr(), squareVertexFuture.GetValue().gpuVertexRegion, sizeof(TexturedVertex), dummyRegion, 0, inputResourceRegions);
