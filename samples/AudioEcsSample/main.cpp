...
int main(int /*argc*/, char* /*argv*/[])
{
    ...
    auto playButtonTransform = NovelRT::Transform(
        NovelRT::Maths::GeoVector2F((1920/2), (1080/2)), 0,
        NovelRT::Maths::GeoVector2F(200, 200));
    playMusicButton =
        runner.getRenderer()->createBasicFillRect(playButtonTransform, 3, NovelRT::Graphics::RGBAColour(0, 255, 0, 255));
    ...
    playNaniButton =
        runner.getRenderer()->createBasicFillRect(naniButtonTransform, 3, NovelRT::Graphics::RGBAColour(160, 0, 0, 255));
    ...
    playLazerButton =
        runner.getRenderer()->createBasicFillRect(lazerButtonTransform, 3, NovelRT::Graphics::RGBAColour(255, 0, 0, 255));
    ...
    playGoatButton =
        runner.getRenderer()->createBasicFillRect(goatButtonTransform, 3, NovelRT::Graphics::RGBAColour(160, 0, 160, 255));
    ...
    auto scheduler = NovelRT::Ecs::Configurator()
    ...
    AudioEmitterComponent waltzComponent = AudioEmitterComponent
        {
            waltzHandle,
            true,
            -1,
    ...
    AudioEmitterComponent nani = AudioEmitterComponent
        {
            naniHandle,
            false,
            0,
    ...
    scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 0, waltzComponent);
    scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 1, nani);
    scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 2, lazer);
    scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 3, goat);

    playMusicInteraction->Interacted += [&] {
        paused = !paused;
        if (paused)
        {
            scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 0, pauseComponent);
            console.logDebugLine("Pausing!");
        }
        else
        {
            scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 0, resumeComponent);
    ...
    playNaniInteraction->Interacted += [&] {
        ...
        scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 1, naniPlay);
    };

    playLazerInteraction->Interacted += [&] {
        console.logInfoLine("Entity Id: 2 - I'ma firin' mah lazar!");
        scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 2, lazerPlay);
    };

    playGoatInteraction->Interacted += [&] {
        console.logInfoLine("Entity Id: 3 - AHHHHHHHHHHHHHHHHHHHHHHHH");
        scheduler.GetComponentCache().GetComponentBuffer<AudioEmitterComponent>().PushComponentUpdateInstruction(0, 3, goatPlay);
    };
