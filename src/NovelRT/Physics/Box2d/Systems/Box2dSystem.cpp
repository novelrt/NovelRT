...
namespace NovelRT::Physics::Box2d
{
    ...
    PhysicsWorld Box2dSystem::AddWorld(NovelRT::Ecs::SystemScheduler* const scheduler,
                                       const NovelRT::Ecs::EntityId worldEntityId,
                                       b2World* const world,
                                       const float stepTime,
                                       const int32 velocityIterations,
                                       const int32 positionIterations)
    {
        auto pWorld = PhysicsWorld{world, velocityIterations, positionIterations, 0, stepTime};
        scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().PushComponentUpdateInstruction(
            0, worldEntityId, pWorld);
        return pWorld;
    }

    void Box2dSystem::RemoveWorld(NovelRT::Ecs::SystemScheduler* const scheduler,
                                  const NovelRT::Ecs::EntityId worldEntityId)
    {
        scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().PushComponentUpdateInstruction(
            0, worldEntityId, PhysicsWorld::DeletedWorld);
    }

    b2Body* Box2dSystem::AddBody(NovelRT::Ecs::SystemScheduler* const scheduler,
                                 const NovelRT::Ecs::EntityId worldEntityId,
                                 const b2BodyDef* const bodyDef)
    {
    ...
    b2Body* Box2dSystem::AddBody(NovelRT::Ecs::SystemScheduler* const scheduler,
                                 const NovelRT::Ecs::EntityId worldEntityId,
                                 const b2BodyDef* const bodyDef,
                                 const b2FixtureDef* const fixtureDef)
    {
        auto pWorld = scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().GetComponent(worldEntityId);
        b2Body* body = pWorld.world->CreateBody(bodyDef);
    ...
    b2Body* Box2dSystem::AddBody(b2World* const world,
                                 const b2BodyDef* const bodyDef,
                                 const b2FixtureDef* const fixtureDef)
    {
    ...
    b2Body* Box2dSystem::AddBodyComponent(NovelRT::Ecs::SystemScheduler* const scheduler,
                                          const NovelRT::Ecs::EntityId worldEntityId,
                                          const NovelRT::Ecs::EntityId entityId,
                                          const b2BodyDef* bodyDef)
    {
        auto pWorld = scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().GetComponent(worldEntityId);
        auto body = pWorld.world->CreateBody(bodyDef);
        PhysicsBody pBody{body};
        scheduler->GetComponentCache().GetComponentBuffer<PhysicsBody>().PushComponentUpdateInstruction(0, entityId,
                                                                                                        pBody);
        return body;
    }
    ...
    void Box2dSystem::AddBodyComponents(NovelRT::Ecs::SystemScheduler* const scheduler,
                                        const NovelRT::Ecs::EntityId* const entityIds,
                                        b2Body** const bodies,
                                        const int32_t bodyCount)
    {
    ...
    void Box2dSystem::RemoveBody(NovelRT::Ecs::SystemScheduler* const scheduler,
                                 const NovelRT::Ecs::EntityId worldEntityId,
                                 b2Body* const body)
    {
        auto pWorld = scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().GetComponent(worldEntityId);
    ...
    void Box2dSystem::RemoveEntityBody(NovelRT::Ecs::SystemScheduler* const scheduler,
                                       b2World* const world,
                                       const NovelRT::Ecs::EntityId entityId)
    {
    ...
    void Box2dSystem::RemoveBodies(NovelRT::Ecs::SystemScheduler* const scheduler,
                                   const NovelRT::Ecs::EntityId worldEntityId,
                                   b2Body** const bodies,
                                   const int32_t bodyCount)
    {
        auto world =
            scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().GetComponent(worldEntityId).world;
        for (int32_t i = 0; i < bodyCount; i++)
        {
            world->DestroyBody(bodies[i]);
        }
    ...
    void Box2dSystem::RemoveEntityBodies(NovelRT::Ecs::SystemScheduler* const scheduler,
                                         b2World* const world,
                                         NovelRT::Ecs::EntityId* const entityIds,
    ...
    b2World* Box2dSystem::GetWorld(NovelRT::Ecs::SystemScheduler* const scheduler,
                                   const NovelRT::Ecs::EntityId worldEntityId)
    {
        auto pWorld = scheduler->GetComponentCache().GetComponentBuffer<PhysicsWorld>().GetComponent(worldEntityId);
        return pWorld.world;
