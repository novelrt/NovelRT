    ...
    void Texture::loadPngAsTexture(const std::string& file)
    {
        if (_textureId.isCreated())
        {
            ...
            throw Exceptions::InvalidOperationException(
                "Unable to continue! Cannot overwrite Texture, please make a new texture.");
        }
        _textureFile = file;
        ...
        if (rawImage == nullptr)
        {
            ...
            throw Exceptions::OutOfMemoryException(
                std::string("Couldn't allocate space for PNG, file: \"").append(file).append("\"."));
        }
        ...
        if (data.rowPointers == nullptr)
        {
            ...
            throw Exceptions::OutOfMemoryException(
                std::string("Could not allocate memory for pixel data from \"").append(file).append("\"."));
        }
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, mode, data.width, data.height, 0, mode, GL_UNSIGNED_BYTE,
                     reinterpret_cast<GLvoid*>(rawImage));
