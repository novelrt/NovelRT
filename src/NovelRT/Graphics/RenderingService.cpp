...
namespace NovelRT::Graphics
{
    ...
    bool RenderingService::initialiseRenderPipeline(bool completeLaunch, Maths::GeoVector2F* const optionalWindowSize)
    {
        ...
        auto windowSize = (optionalWindowSize == nullptr) ? _windowingService->getWindowSize()
                                                          : *optionalWindowSize; // lol this is not safe
        ...
        if (completeLaunch)
        {
            ...
            if (!gladLoadGLES2Loader(reinterpret_cast<GLADloadproc>(glfwGetProcAddress)))
            {
                ...
                throw Exceptions::InitialisationFailureException(
                    "Unable to continue! The engine cannot start without glad.");
            }
    ...
    ShaderProgram RenderingService::loadShaders(const std::string& vertexFileName, const std::string& fragmentFileName)
    {
        ...
        {
            _logger.logError("Target Vertex Shader file cannot be opened! Please ensure the path is correct and that "
                             "the file is not locked.");
            throw Exceptions::FileNotFoundException("Target Vertex Shader file cannot be opened! Please ensure the "
                                                    "path is correct and that the file is not locked.",
                                                    vertexFileName);
        }
        ...
        {
            _logger.logError("Target Fragment Shader file cannot be opened! Please ensure the path is correct and that "
                             "the file is not locked.");
            throw Exceptions::FileNotFoundException("Target Fragment Shader file cannot be opened! Please ensure the "
                                                    "path is correct and that the file is not locked.",
                                                    fragmentFileName);
        }
    ...
    int32_t RenderingService::initialiseRendering()
    {
        if (!initialiseRenderPipeline())
        {
            ...
            throw Exceptions::InitialisationFailureException(
                "Unable to continue! The engine cannot start without GLAD/GLFW3.");
        }
    ...
    void RenderingService::beginFrame() const
    {
        ...
        glClearColor(_framebufferColour.getRScalar(), _framebufferColour.getGScalar(), _framebufferColour.getBScalar(),
                     _framebufferColour.getAScalar());
    ...
    std::shared_ptr<Texture> RenderingService::getTexture(const std::string& fileTarget)
    {
        if (!fileTarget.empty())
        {
            for (auto& pair : _textureCache)
            {
                ...
                if (result->getTextureFile() != fileTarget)
                    continue;
    ...
    std::shared_ptr<FontSet> RenderingService::getFontSet(const std::string& fileTarget, float fontSize)
    {
        if (!fileTarget.empty())
        {
            ...
            {
                auto result = pair.second.lock();
                if (result->getFontFile() != fileTarget || result->getFontSize() != fontSize)
                    continue;
