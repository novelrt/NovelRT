...
SpriteAnimator::SpriteAnimator(NovelRunner *runner, Graphics::ImageRect *rect) noexcept
    : _accumulatedDelta(0), _currentFrameIndex(0), _runner(runner), _rect(rect),
      _animatorState(AnimatorPlayState::Stopped), _animationUpdateHandle(Utilities::EventHandler<Timing::Timestamp>(
                                                      [=](Timing::Timestamp delta) { constructAnimation(delta); })),
      _currentState(nullptr), _logger(Utilities::Misc::CONSOLE_LOG_GENERIC)
{
}
...
void SpriteAnimator::constructAnimation(Timing::Timestamp delta)
{
    ...
    case AnimatorPlayState::Playing: {
        ...
        if (_currentState->frames().size() > _currentFrameIndex &&
            _currentState->frames().at(_currentFrameIndex).duration() <= _accumulatedDelta)
        {
