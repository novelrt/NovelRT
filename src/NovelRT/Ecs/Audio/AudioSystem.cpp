...
namespace NovelRT::Ecs::Audio
{
        ...
        for (auto [entity, emitter] : emitters)
        {
            switch(emitter.state)
            {
                case EmitterState::ToPlay :
                {
                    ...
                    emitters.PushComponentUpdateInstruction(entity, AudioEmitterComponent{emitter.handle,emitter.isMusic,emitter.numberOfLoops,emitter.position,emitter.volume,EmitterState::Playing});
                    break;
                }
                case EmitterState::ToStop :
                {
                    ...
                    emitters.PushComponentUpdateInstruction(entity, AudioEmitterComponent{emitter.handle,emitter.isMusic,emitter.numberOfLoops,emitter.position,emitter.volume,EmitterState::Stopped});
                    break;
                }
                case EmitterState::ToPause :
                {
                    ...
                    if(emitter.isMusic)
                    {
                        ...
                        emitters.PushComponentUpdateInstruction(entity, AudioEmitterComponent{emitter.handle,emitter.isMusic,emitter.numberOfLoops,emitter.position,emitter.volume, EmitterState::Paused});
                    }
                    break;
                }
    ...
    uint32_t AudioSystem::CreateAudio(std::string& fileName, bool isMusic)
    {
        ...
        //0 is not valid here since we're working with only positive numbers... not that anyone should be loading thousands of sounds.
        uint32_t value = 0U;
        if(isMusic)
        {
