...
namespace NovelRT::ResourceManagement::Desktop
{
    ...
    AudioMetadata DesktopResourceLoader::LoadAudioFrameData(const std::string& soundName)
    {
        ...
        if (file == INVALID_HANDLE_VALUE)
        {
            ...
            throw NovelRT::Exceptions::IOException(filePath.string(), msg);
        }
        ...
        uint32_t chunkPosition = 0;

        // Need to put this somewhere else and allow to change to BE
        uint32_t headerRIFF = 0x46464952;   // FFIR
        uint32_t headerWAVE = 0x45564157;   // EVAW
        uint32_t headerFMT  = 0x20746D66;   //  tmf
        uint32_t headerDATA = 0x61746164;   // atad
        bool res = FindWaveChunk(file, headerRIFF, chunkSize, chunkPosition);
        if(!res)
        {
            // Should this really be an exception? Will determine before merging.
            throw NovelRT::Exceptions::NotSupportedException("Could not find RIFF header in provided wave file.");
        }

        uint32_t fileType = 0;
        ReadChunk(file, &fileType, sizeof(uint32_t), chunkPosition);
        if(fileType != headerWAVE)
        {
            throw NovelRT::Exceptions::NotSupportedException("Provided wave file is not defined as type WAVE - unable to load.");
        }

        //Get the format information from the wave file.
        res = FindWaveChunk(file, headerFMT, chunkSize, chunkPosition);
        if(!res)
        {
            // Should this really be an exception? Will determine before merging.
        ...
        return AudioMetadata{data, chunkSize, wfx};
    }
    ...
    bool DesktopResourceLoader::FindWaveChunk(HANDLE file, uint32_t chunkHeader, uint32_t& chunkSize, uint32_t& chunkDataPosition)
    {
        HRESULT hr = S_OK;
        if(SetFilePointer(file, 0, NULL, FILE_BEGIN) == INVALID_SET_FILE_POINTER)
        {
        ...
        while(hr == S_OK)
        {
            if(ReadFile(file, &header, sizeof(chunkHeader), &bytesRead, NULL) == 0)
            {
                hr = HRESULT_FROM_WIN32(GetLastError());
            }

            if(ReadFile(file, &size, sizeof(header), &bytesRead, NULL) == 0)
            {
                hr = HRESULT_FROM_WIN32(GetLastError());
            }
            ...
            switch(header)
            {
                case 0x46464952: //FFIR - RIFF header, LE
                {
                    dataSize = size;
                    size = 4;
                    if(ReadFile(file, &fileType, sizeof(fileType), &bytesRead, NULL) == 0)
                    {
                        hr = HRESULT_FROM_WIN32(GetLastError());
                    }
                    break;
                ...
                default:
                {
                    if(SetFilePointer(file, chunkSize, NULL, FILE_CURRENT) == INVALID_SET_FILE_POINTER)
                    {
                        _com_error err(HRESULT_FROM_WIN32(GetLastError()));
                        std::string msg = err.ErrorMessage();
                        throw NovelRT::Exceptions::InvalidOperationException(msg);
    ...
    void DesktopResourceLoader::ReadChunk(HANDLE file, void* buffer, uint32_t bufferSize, uint32_t bufferOffset)
    {
        ...
        if(ReadFile(file, buffer, bufferSize, NULL, NULL) == 0)
        {
            _com_error err(HRESULT_FROM_WIN32(GetLastError()));
            std::string msg = err.ErrorMessage();
            throw NovelRT::Exceptions::InvalidOperationException(msg);
