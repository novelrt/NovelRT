...
TEST_F(InteropQuadTreeTest, deleteReturnsSuccessWhenGivenValidHandle) {
  ...
  EXPECT_EQ(Nrt_QuadTree_delete(tree, nullptr), NRT_SUCCESS);
}

TEST_F(InteropQuadTreeTest, deleteReturnsNullptrFailureWhenGivenNullptr) {
  const char* errorOutput = nullptr;

  ASSERT_EQ(Nrt_QuadTree_delete(nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(Nrt_getErrMsgIsNullptr(), errorOutput);
}
...
TEST_F(InteropQuadTreeTest, deleteReturnsAlreadyDeletedOrRemovedFailureWhenPointIsBeingHeldOntoElsewhereAndHasAlreadyBeenDeletedFromCache) {
  ...
  ASSERT_EQ(Nrt_QuadTree_delete(reinterpret_cast<NrtQuadTree>(handle.get()), &errorOutput), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_delete(reinterpret_cast<NrtQuadTree>(handle.get()), &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(Nrt_getErrMsgIsAlreadyDeletedOrRemoved(), errorOutput);
}
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsReturnsValidPointVectorHandleAndCanAlsoBeDeleted) {
  ...
  int32_t result = Nrt_QuadTree_getIntersectingPoints(_quadTree, getCenteredBoundsC(TEST_WIDTH, TEST_HEIGHT), &vec, nullptr);
  EXPECT_EQ(result, NRT_SUCCESS);
  EXPECT_EQ(Nrt_PointVector_delete(vec, nullptr), NRT_SUCCESS);
}
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsReturnsNullptrFailureWhenGivenNullPtrForInput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(nullptr, expectedBounds, &outputVector, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsReturnsNullptrFailureWhenGivenNullPtrForOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(_quadTree, expectedBounds, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsReturnsNullptrFailureWhenGivenNullPtrForBothInputAndOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(nullptr, expectedBounds, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getBoundsGetsCorrectBounds) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getBounds(_quadTree, &actualBounds, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, getBoundsReturnsNullptrFailureWhenGivenNullPtrForInput) {
  ...
  const char* errorOutput = nullptr;
  ASSERT_EQ(Nrt_QuadTree_getBounds(nullptr, &outputBounds, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getBoundsReturnsNullptrFailureWhenGivenNullPtrForOutput) {
  NrtGeoBounds outputBounds = Nrt_GeoBounds_zero();
  const char* errorOutput = nullptr;
  ASSERT_EQ(Nrt_QuadTree_getBounds(_quadTree, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getBoundsReturnsNullptrFailureWhenGivenNullPtrForBothInputAndOutput) {
  NrtGeoBounds outputBounds = Nrt_GeoBounds_zero();
  const char* errorOutput = nullptr;
  ASSERT_EQ(Nrt_QuadTree_getBounds(nullptr, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}

TEST_F(InteropQuadTreeTest, getPointCountReturnsZeroWhenTreeHasNoPoints) {
  size_t actualSize = 0;
  ASSERT_EQ(Nrt_QuadTree_getPointCount(_quadTree, &actualSize, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, getPointCountReturnsNullptrFailureWhenGivenNullptrForInput) {
  ...
  const char* errorOutput = nullptr;
  size_t size = 0;
  ASSERT_EQ(Nrt_QuadTree_getPointCount(nullptr, &actualSize, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}

TEST_F(InteropQuadTreeTest, getPointCountReturnsNullptrFailureWhenGivenNullptrForOutput) {
  const char* errorOutput = nullptr;
  ASSERT_EQ(Nrt_QuadTree_getPointCount(_quadTree, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}

TEST_F(InteropQuadTreeTest, getPointCountReturnsNullptrFailureWhenGivenNullptrForBothInputAndOutput) {
  const char* errorOutput = nullptr;
  ASSERT_EQ(Nrt_QuadTree_getPointCount(nullptr, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getPointMethodsReturnsNullptrWhenTreeHasNoChildren) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getTopLeft(_quadTree, &topLeft, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getTopRight(_quadTree, &topRight, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomLeft(_quadTree, &bottomLeft, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomRight(_quadTree, &bottomRight, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, getPointReturnsNullForTooLargeIndex) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 0, &point0, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 1, &point1, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 2, &point2, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 3, &point3, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, getPointReturnsNullptrFailureWhenGivenNullptrForInput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(nullptr, 0, &point, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getPointReturnsNullptrFailureWhenGivenNullptrForOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 0, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getPointReturnsNullptrFailureWhenGivenNullptrForInputAndOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(nullptr, 0, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, insertOneReturnsTrue) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point0, &result, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, insertReturnsNullptrFailureWhenGivenNullptrForInput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(nullptr, point0, &result, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, insertReturnsNullptrFailureWhenGivenNullptrForOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point0, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, insertReturnsNullptrFailureWhenGivenNullptrForInputAndOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(nullptr, point0, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, insertFourDoesNotSubdivide) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point0, &point0InsertResult, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point1, &point1InsertResult, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point2, &point2InsertResult, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point3, &point3InsertResult, nullptr), NRT_SUCCESS);
  EXPECT_TRUE(point3InsertResult);

  size_t outputSizeResult = 0u;
  ASSERT_EQ(Nrt_QuadTree_getPointCount(_quadTree, &outputSizeResult, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 0, &outputPointResult0, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 1, &outputPointResult1, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 2, &outputPointResult2, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 3, &outputPointResult3, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_getTopLeft(_quadTree, &outputCornerTreeTopLeft, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getTopRight(_quadTree, &outputCornerTreeTopRight, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomLeft(_quadTree, &outputCornerTreeBottomLeft, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomRight(_quadTree, &outputCornerTreeBottomRight, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, insertFiveDoesSubdivideAndPointsAreCorrect) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point4, &point4InsertResult, nullptr), NRT_SUCCESS);
  ...
  int32_t treeResultTopLeft = Nrt_QuadTree_getTopLeft(_quadTree, &treeOutputTopLeft, nullptr);
  int32_t treeResultTopRight = Nrt_QuadTree_getTopRight(_quadTree, &treeOutputTopRight, nullptr);
  int32_t treeResultBottomLeft = Nrt_QuadTree_getBottomLeft(_quadTree, &treeOutputBottomLeft, nullptr);
  int32_t treeResultBottomRight = Nrt_QuadTree_getBottomRight(_quadTree, &treeOutputBottomRight, nullptr);
  ...
  int32_t pointCountOperationResultTopLeft = Nrt_QuadTree_getPointCount(treeOutputTopLeft, &outputChildTreeSizeTopLeft, nullptr);  
  int32_t pointCountOperationResultTopRight = Nrt_QuadTree_getPointCount(treeOutputTopRight, &outputChildTreeSizeTopRight, nullptr);
  int32_t pointCountOperationResultBottomLeft = Nrt_QuadTree_getPointCount(treeOutputBottomLeft, &outputChildTreeSizeBottomLeft, nullptr);
  int32_t pointCountOperationResultBottomRight = Nrt_QuadTree_getPointCount(treeOutputBottomRight, &outputChildTreeSizeBottomRight, nullptr);
  ...
  int32_t treeResultTwoTopLeft = Nrt_QuadTree_getTopLeft(_quadTree, &treeOutputTwoTopLeft, nullptr);
  int32_t treeResultTwoTopRight = Nrt_QuadTree_getTopRight(_quadTree, &treeOutputTopRight, nullptr);
  int32_t treeResultTwoBottomLeft = Nrt_QuadTree_getBottomLeft(_quadTree, &treeOutputBottomLeft, nullptr);
  int32_t treeResultTwoBottomRight = Nrt_QuadTree_getBottomRight(_quadTree, &treeOutputBottomRight, nullptr);
  ...
  Nrt_QuadTree_getPoint(treeOutputTopLeft, 0, &outputPointResultTopLeft, nullptr);
  Nrt_QuadTree_getPoint(treeOutputTopRight, 0, &outputPointResultTopRight, nullptr);
  Nrt_QuadTree_getPoint(treeOutputBottomLeft, 0, &outputPointResultBottomLeft, nullptr);
  Nrt_QuadTree_getPoint(treeOutputBottomRight, 0, &outputPointResultBottomRight, nullptr);
  Nrt_QuadTree_getPoint(treeOutputTopLeft, 1, &outputPointResultTopLeftTwo, nullptr);
...
TEST_F(InteropQuadTreeTest, insertFiveDoesSubdivideAndBoundsAreCorrect) {
  ...
  int32_t boundsResultTopLeft = Nrt_QuadTree_getBounds(treeOutputTopLeft, &outputBoundsTopLeft, nullptr);
  int32_t boundsResultTopRight = Nrt_QuadTree_getBounds(treeOutputTopRight, &outputBoundsTopRight, nullptr);
  int32_t boundsResultBottomLeft = Nrt_QuadTree_getBounds(treeOutputBottomLeft, &outputBoundsBottomLeft, nullptr);
  int32_t boundsResultBottomRight = Nrt_QuadTree_getBounds(treeOutputBottomRight, &outputBoundsBottomRight, nullptr);
...
TEST_F(InteropQuadTreeTest, insertOutOfBoundsReturnsFalse) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point0, &outputBool, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, PointVector_getSizeReturnsCorrectValue) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getBounds(_quadTree, &inputGeoBounds, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(_quadTree, inputGeoBounds, &resultVector, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getSize(resultVector, &outputVectorSize, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, PointVector_deleteReturnsNullptrFailureWhenGivenNullptr) {
  ...
  ASSERT_EQ(Nrt_PointVector_delete(nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsReturnsAllPoints) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getBounds(_quadTree, &outputGeoBounds, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(_quadTree, outputGeoBounds, &resultVector, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getSize(resultVector, &outputVectorSize, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_PointVector_getPointFromIndex(resultVector, 0, &outputPoint0, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getPointFromIndex(resultVector, 1, &outputPoint1, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getPointFromIndex(resultVector, 2, &outputPoint2, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getPointFromIndex(resultVector, 3, &outputPoint3, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_PointVector_getPointFromIndex(resultVector, 4, &outputPoint4, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, removeExistingReturnsTrue) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point0, &outputInsertBool, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_tryRemove(_quadTree, point0, &outputRemoveBool, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, removeNoneExistingReturnsFalse) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryRemove(_quadTree, point0, &outputBool, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, removeReturnsNullptrFailureWhenGivenNullptrForInput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryRemove(nullptr, point, &outputBool, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, removeReturnsNullptrFailureWhenGivenNullptrForOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryRemove(_quadTree, point, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, removeReturnsNullptrFailureWhenGivenNullptrForInputAndOutput) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryRemove(nullptr, point, nullptr, &errorOutput), NRT_FAILURE_UNKOWN);
  EXPECT_EQ(errorOutput, Nrt_getErrMsgIsNullptr());
}
...
TEST_F(InteropQuadTreeTest, removeOneCausesMergeWhenAdjacentPointCountLessThan5) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryRemove(_quadTree, point4, &tryRemoveOutputBool, nullptr), NRT_SUCCESS);
  EXPECT_TRUE(tryRemoveOutputBool);

  size_t outputPointCount = 0u;

  ASSERT_EQ(Nrt_QuadTree_getPointCount(_quadTree, &outputPointCount, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 0, &newOutputPoint0, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 1, &newOutputPoint1, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 2, &newOutputPoint2, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getPoint(_quadTree, 3, &newOutputPoint3, nullptr), NRT_SUCCESS);
  ...
  ASSERT_EQ(Nrt_QuadTree_getTopLeft(_quadTree, &newTopLeftTree, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getTopRight(_quadTree, &newTopRightTree, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomLeft(_quadTree, &newBottomLeftTree, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getBottomRight(_quadTree, &newBottomRightTree, nullptr), NRT_SUCCESS);
...
TEST_F(InteropQuadTreeTest, removeOneDoesNotCauseMergeWhenAdjacentPointCountMoreThan4) {
  ...
  ASSERT_EQ(Nrt_QuadTree_tryInsert(_quadTree, point5, &point5InsertResult, nullptr), NRT_SUCCESS);
  EXPECT_TRUE(point5InsertResult);

  int32_t tryRemoveOutputBool = NRT_FALSE;

  ASSERT_EQ(Nrt_QuadTree_tryRemove(_quadTree, point5, &tryRemoveOutputBool, nullptr), NRT_SUCCESS);
  EXPECT_TRUE(tryRemoveOutputBool);
...
TEST_F(InteropQuadTreeTest, getIntersectingPointsForQuadTreeBoundsReturnsAll) {
  ...
  ASSERT_EQ(Nrt_QuadTree_getBounds(_quadTree, &outputGeoBounds, nullptr), NRT_SUCCESS);
  ASSERT_EQ(Nrt_QuadTree_getIntersectingPoints(_quadTree, outputGeoBounds, &outputVector, nullptr), NRT_SUCCESS);  
