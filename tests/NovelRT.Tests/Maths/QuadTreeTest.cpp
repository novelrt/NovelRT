...
TEST_F(QuadTreeTest, insertFiveDoesSubdivideAndBoundsAreCorrect) {
  ...
  EXPECT_EQ(_quadTree->getTopLeft()->getBounds(), GeoBounds(GeoVector2F(-TEST_WIDTH / 4, TEST_HEIGHT / 4), expectedSize, 0));
  EXPECT_EQ(_quadTree->getTopRight()->getBounds(), GeoBounds(GeoVector2F(TEST_WIDTH / 4, TEST_HEIGHT / 4), expectedSize, 0));
  EXPECT_EQ(_quadTree->getBottomLeft()->getBounds(), GeoBounds(GeoVector2F(-TEST_WIDTH / 4, -TEST_HEIGHT / 4), expectedSize, 0));
  EXPECT_EQ(_quadTree->getBottomRight()->getBounds(), GeoBounds(GeoVector2F(TEST_WIDTH / 4, -TEST_HEIGHT / 4), expectedSize, 0));
}
