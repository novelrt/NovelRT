...
TEST_F(EntityCacheTest, AddEntityAddsEntityCorrectly)
{
    ...
    EXPECT_NE(
        std::find(cache.GetRegisteredEntities().begin(), cache.GetRegisteredEntities().end(), NovelRT::Core::Atom(10)),
...
TEST_F(EntityCacheTest, AddEntityAddsEntityCorrectlyMultipleTimes)
{
    ...
    EXPECT_NE(
        ...
        cache.GetRegisteredEntities().end());
    EXPECT_NE(
        std::find(cache.GetRegisteredEntities().begin(), cache.GetRegisteredEntities().end(), NovelRT::Core::Atom(20)),
        cache.GetRegisteredEntities().end());
}
...
TEST_F(EntityCacheTest, ApplyEntityDeletionRequestsToRegisteredEntitiesAppliesEntityDeletionCorrectly)
{
    ...
    EXPECT_NE(
        ...
        cache.GetRegisteredEntities().end());
    EXPECT_NE(
        std::find(cache.GetRegisteredEntities().begin(), cache.GetRegisteredEntities().end(), NovelRT::Core::Atom(30)),
        cache.GetRegisteredEntities().end());
}
