...
// this aint working because of forcing centralized header
...
namespace NovelRT::Physics::Box2d
{
    class Box2dSystem{
        private:
            static void UpdateWorld(NovelRT::Timing::Timestamp delta, NovelRT::Ecs::Catalogue catalogue);
            static void UpdatePosition(NovelRT::Timing::Timestamp delta, NovelRT::Ecs::Catalogue catalogue);
        public:
            static NovelRT::Ecs::EntityId CreateWorld(NovelRT::Ecs::SystemScheduler* scheduler, b2Vec2 gravity, float stepTime, int32 velocityIterations = 6, int32 positionIterations = 2);
            
            static b2Body* AddBody(NovelRT::Ecs::SystemScheduler* scheduler, NovelRT::Ecs::EntityId worldId, const b2BodyDef* bodyDef);
            static b2Body* AddBody(NovelRT::Ecs::SystemScheduler* scheduler, NovelRT::Ecs::EntityId worldId, const b2BodyDef* bodyDef, const b2FixtureDef* fixtureDef);
            static b2Body* AddBody(NovelRT::Ecs::SystemScheduler* scheduler, NovelRT::Ecs::EntityId worldId, const b2BodyDef* bodyDef, const b2FixtureDef** fixtureDef, int32_t fixtureCount);
            ...
            static void RegisterSystem(NovelRT::Ecs::SystemScheduler* scheduler){
                scheduler->RegisterSystem(UpdateWorld);
